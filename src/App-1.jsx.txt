...
// ── Auth Modal ────────────────────────────────────────────────
function AuthModal({ onClose, onAuth, dark, mode: initialMode = "login" }) {
  ...
  const input = dark ? "bg-gray-800 border-gray-700 text-gray-100 placeholder-gray-500" : "bg-gray-50 border-gray-300 text-gray-900 placeholder-gray-400";
  ...
  const handleSubmit = () => {
    ...
    const users = JSON.parse(localStorage.getItem("scamcheck_users") || "{}");

    if (mode === "register") {
      if (users[email]) { setError("Este correo ya está registrado"); return; }
      users[email] = { password, premium: false, analyses: 0 };
      localStorage.setItem("scamcheck_users", JSON.stringify(users));
    } else {
    ...
    localStorage.setItem("scamcheck_session", JSON.stringify({ email, premium: users[email].premium }));
    onAuth({ email, premium: users[email].premium });
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className={`w-full max-w-sm rounded-3xl border shadow-2xl p-8 ${bg} ${text_}`}
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className={`text-sm mt-1 ${muted}`}>
              {mode === "login" ? "Bienvenido de vuelta" : "Empieza a protegerte hoy"}
            </p>
          </div>
          <button onClick={onClose} className={`p-2 rounded-xl ${dark ? "hover:bg-gray-800" : "hover:bg-gray-100"}`}>
            <X className="w-4 h-4"/>
          </button>
        </div>
        ...
        <div className="space-y-3 mb-4">
          <input type="email" placeholder="Correo electrónico" value={email}
            ...
            className={`w-full px-4 py-3 rounded-2xl border text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 ${input}`}/>
        ...
        <button onClick={handleSubmit}
          className="w-full py-3 rounded-2xl bg-blue-600 text-white font-bold text-sm hover:bg-blue-500 transition-all active:scale-95 mb-4">
...
// ── Limit Modal ───────────────────────────────────────────────
function LimitModal({ onClose, onLogin, onPremium, dark }) {
  const bg = dark ? "bg-gray-900 border-gray-800" : "bg-white border-gray-200";
  const text_ = dark ? "text-gray-100" : "text-gray-900";
  const muted = dark ? "text-gray-400" : "text-gray-500";
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className={`w-full max-w-sm rounded-3xl border shadow-2xl p-8 text-center ${bg} ${text_}`}
        style={{ animation: "fadeInUp 0.3s ease" }}>
        <div className="w-16 h-16 rounded-2xl bg-amber-500/15 flex items-center justify-center mx-auto mb-4">
          <AlertTriangle className="w-8 h-8 text-amber-500"/>
        </div>
        <h2 className="text-xl font-black mb-2">Límite alcanzado</h2>
        <p className={`text-sm mb-6 ${muted}`}>
          Has usado tus <strong>20 análisis gratuitos</strong>. Regístrate para obtener más o hazte Premium para análisis ilimitados.
        </p>
        <div className="space-y-3">
          <button onClick={onPremium}
            className="w-full py-3 rounded-2xl bg-blue-600 text-white font-bold text-sm hover:bg-blue-500 transition-all active:scale-95">
            🚀 Hacerme Premium · $10.000 COP/mes
          </button>
          <button onClick={onLogin}
            className={`w-full py-3 rounded-2xl border font-semibold text-sm transition-all active:scale-95 ${dark ? "border-gray-700 hover:bg-gray-800" : "border-gray-300 hover:bg-gray-100"}`}>
            Iniciar sesión / Registrarse
          </button>
          <button onClick={onClose} className={`text-sm ${muted} hover:underline`}>
            Cerrar
          </button>
        </div>
      </div>
...
// ── Donation Section ──────────────────────────────────────────
function DonationSection({ dark, muted, card }) {
  const [tab, setTab] = useState("qr");
  const [copied, setCopied] = useState(false);
  const NEQUI_KEY = "0092589899";
  ...
  return (
    <div className="p-6">
      {/* Tabs */}
      <div className={cn("flex rounded-2xl p-1 mb-6", dark ? "bg-gray-800" : "bg-gray-100")}>
      ...
      {tab === "qr" ? (
        <div className="text-center">
          <p className={cn("text-sm mb-4", muted)}>
            Abre Nequi → toca el ícono de escanear → apunta al QR
          </p>
          <div className="relative inline-block">
            <div className="absolute inset-0 rounded-3xl opacity-20 blur-xl bg-blue-600"/>
            <div className={cn("relative rounded-3xl p-4 border", dark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200")}>
              <img src="/nequi-qr.png" alt="QR Nequi ScamCheck"
        ...
        <div className="text-center">
          <p className={cn("text-sm mb-6", muted)}>
            Abre Nequi → "Enviar dinero" → "A llave de negocios" → pega el número
          </p>
          <div className={cn("rounded-2xl border p-6 mb-4", dark ? "bg-gray-800 border-gray-700" : "bg-gray-50 border-gray-200")}>
            <p className={cn("text-xs font-bold uppercase tracking-widest mb-3", muted)}>
              Llave Nequi Negocios
            </p>
      ...
      <div className={cn("mt-6 p-3 rounded-2xl text-center text-xs", dark ? "bg-gray-800/60" : "bg-gray-100")}>
        <span className={muted}>Dona el monto que quieras · Cada peso ayuda a mantener ScamCheck gratuito</span>
      </div>
    </div>
  );
}
...
export default function ScamCheck() {
  ...
  useEffect(() => {
    const session = localStorage.getItem("scamcheck_session");
  ...
  const handleLogout = () => {
    localStorage.removeItem("scamcheck_session");
    setUser(null);
  };
  ...
  return (
    <div className={cn("min-h-screen font-sans transition-colors duration-300", bg, text_)}
      ...
      {/* ── MODALS ── */}
      {showAuth && (
        <AuthModal dark={dark} mode={authMode}
          ...
          <div className="flex items-center gap-3">
            ...
            {user ? (
              <div className="flex items-center gap-2">
                <div className={cn("hidden sm:flex items-center gap-2 text-sm px-3 py-1.5 rounded-xl", dark ? "bg-gray-800" : "bg-gray-100")}>
                  <User className="w-3.5 h-3.5 text-blue-500"/>
                  <span className={cn("max-w-24 truncate", muted)}>{user.email.split("@")[0]}</span>
                  {user.premium && <span className="text-xs bg-blue-600 text-white px-1.5 py-0.5 rounded-full font-bold">PRO</span>}
                </div>
                <button onClick={handleLogout}
                  className={cn("text-sm font-medium px-3 py-2 rounded-lg transition-colors", muted, dark ? "hover:bg-gray-800" : "hover:bg-gray-100")}>
                  Salir
                </button>
              </div>
            ) : (
              <button onClick={() => { setAuthMode("login"); setShowAuth(true); }}
                className={cn("hidden sm:block text-sm font-medium px-4 py-2 rounded-lg transition-colors", muted,
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-8 pb-8 border-b"
                ...
                <div>
                  ...
                  <div className={cn("inline-block text-xs px-2.5 py-1 rounded-lg font-semibold",
                    result.risk >= 70 ? "bg-red-500/15 text-red-500"
                      : result.risk >= 40 ? "bg-amber-500/15 text-amber-500"
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {[
              ...
              <div key={step} className={cn("relative p-6 rounded-3xl border transition-all hover:-translate-y-1 hover:shadow-xl", card)}>
      ...
      <section className="py-24 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {[
              { title: "WhatsApp falso", badge: "94% riesgo", msg: '"Tu cuenta fue suspendida. Verifica en: whatsapp-verify-now.xyz..."', signals: ["Dominio falso", "Urgencia artificial", "Suplantación de marca"], color: "#EF4444" },
              { title: "Phishing bancario", badge: "89% riesgo", msg: '"Su tarjeta fue bloqueada. Ingrese sus datos en: banco-seguro-co.ml"', signals: ["URL maliciosa", "Solicita datos bancarios", "Crea falsa alarma"], color: "#F97316" },
              { title: "Inversión falsa", badge: "91% riesgo", msg: '"Invierte $100 y gana $3000 en 24 horas. Grupo VIP de cripto..."', signals: ["Promesa irreal", "Esquema Ponzi", "Sin regulación"], color: "#EF4444" },
            ].map(({ title, badge, msg, signals, color }) => (
              <div key={title} className={cn("p-6 rounded-3xl border", card)}>
                <div className="flex items-center justify-between mb-4">
                  <span className="font-bold">{title}</span>
                  <span className="text-xs font-bold px-2 py-1 rounded-lg" style={{ background: `${color}15`, color }}>{badge}</span>
                </div>
                <p className={cn("text-xs leading-relaxed mb-4 p-3 rounded-xl italic", dark ? "bg-gray-800/60 text-gray-400" : "bg-gray-100 text-gray-600")}>{msg}</p>
        ...
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className={cn("text-base leading-relaxed max-w-lg mx-auto", muted)}>
              ScamCheck es completamente gratuito y sin anuncios. Si te fue útil y quieres ayudarnos a mantenerlo vivo, puedes hacer una donación voluntaria por Nequi.
            </p>
          </div>

          <div className={cn("rounded-3xl border overflow-hidden", card)}>
            {/* Header */}
          ...
          <p className={cn("text-center text-xs mt-6", muted)}>
            Cualquier aporte, por pequeño que sea, nos ayuda a proteger a más personas en Colombia 🇨🇴
          </p>
        </div>
      </section>
